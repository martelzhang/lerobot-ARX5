        ...
        while True:
            ...
            for key_stroke in press_events:
                if key_stroke == KeyCode(char="t"):
                    ...
                    gain.kp()[:] = 0.0  # 禁用所有关节位置控制
                    gain.kd()[:] = 0.0  # 关节阻尼降低，更容易掰动
                    gain.gripper_kp = 0.0  # 完全禁用夹爪位置控制
                    gain.gripper_kd = 0.0  # 完全禁用夹爪阻尼
                    controller.set_gain(gain)  # 应用自定义阻尼设置

                    # 设置夹爪扭矩为0，确保没有扭矩输出
                    # joint_cmd = controller.get_joint_cmd()
                    # joint_cmd.gripper_torque = 0.0
                    # controller.set_joint_cmd(joint_cmd)

                    # 验证增益设置
                    verify_gain = controller.get_gain()
                    print(f"Applied gains - Joint kp: {verify_gain.kp()}")
                    print(f"Applied gains - Joint kd: {verify_gain.kd()}")
                    print(f"Applied gains - Gripper kp: {verify_gain.gripper_kp}")
                    print(f"Applied gains - Gripper kd: {verify_gain.gripper_kd}")

                    # 验证夹爪扭矩设置
                    verify_cmd = controller.get_joint_cmd()
                    print(f"Applied gripper torque: {verify_cmd.gripper_torque}")
                    ...
                    print(
                        "Note: Gripper should be manually movable now. If not, check gripper settings."
                    )
